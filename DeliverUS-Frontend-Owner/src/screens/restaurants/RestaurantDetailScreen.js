...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const [mensajeColor, setMensajeColor] = useState('black')
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMensajeColor(prev => prev === 'black' ? 'blue' : 'black')
    }, 1000)
  ...
  const renderHeader = () => {
    return (
      <View>
        ...
        { (restaurant.mensaje!== null) &&
          <TextSemiBold textStyle={{color: mensajeColor, fontSize: 18, textAlign: 'center', marginVertical: 10}}>
            {restaurant.mensaje}
          </TextSemiBold>
        }
